...
class MultiAgentsSupervisorOperatorsEnv(MultiAgentEnv):
    def __init__(self, env_config):
        ...
        self.goal_positions = {
             self.agents_ids[0]: ([1,1,1],[1,4,7]) 
            ,self.agents_ids[1]: ([0,1,1],[2,1,3]) 
            ,self.agents_ids[2]: ([0,1,2],[3,4,5]) 
            ,self.agents_ids[3]: ([2,2,2],[6,7,8]) 
            }
        self.check_goal = self.goal_positions
    ...
    def step(self, action_dict):
        ...
        for agent_id, action in action_dict.items():
            ...
            for i in range(0,len(self.check_goal [agent_id][0])) :
                ...
                if (now_x,now_y)==(goal_x,goal_y) :
                    #print("i = " + str(i))
                    #print("len(self.goal_positions) = "+ str(len(self.check_goal[agent_id][0])))
                    #print("check_goal = "+ str(self.check_goal [agent_id]))
                    rewards[agent_id]=10  
                    #print("win")
                    #sleep(1)
                    ...
                    #print("goal_uncheck = " + str(goal_uncheck))
                    if goal_uncheck == 0 : 
        ...
        #print("goal = " + str(self.check_goal ))
    ...
    def _get_observation(self,agent_id):
        ...
        observation+=self.goal_positions[agent_id][0]
        observation+=self.goal_positions[agent_id][1]
        
        #print("self.goal_positions[agent_id]" + str(self.goal_positions[agent_id]))
        return observation
...
if __name__ == "__main__":
    ...
    obs_supervisor = spaces.Box(low=0, high=9, shape=(8,))
    obs_operator = spaces.Box(low=0, high=9, shape=(8,))
    acti = spaces.Discrete(4)
    policies = {
        "supervisor_policy": (None,obs_supervisor,acti, {}),# <- use default class & infer obs-/act-spaces from env.
        "operator_0": (None,obs_operator,acti, {}),# <- use default class & infer obs-/act-spaces from env.
    ...
    def policy_mapping_fn(agent_id, episode, worker, **kwargs):
        ...
        elif agent_id == "operator_0":
            #print("2")
            return "operator_0"
        elif agent_id == "operator_1":
            #print("2")
            return "operator_1"
        elif agent_id == "operator_2":
            #print("2")
            return "operator_2"
