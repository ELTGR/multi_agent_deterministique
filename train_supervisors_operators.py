...
class MultiAgentsSupervisorOperatorsEnv(MultiAgentEnv):
    def __init__(self, env_config):
        ...
        self.goal_positions = {
             self.agents_ids[0]: ([1,1,1,4,4,4,7,7,7,10,10,10],[1,4,7,1,4,7,1,4,7,1,4,7]) 
...
if __name__ == "__main__":
    ...
    obs_supervisor = spaces.Box(low=0, high=12, shape=(26,))
    obs_operator = spaces.Box(low=0, high=12, shape=(8,))
    acti = spaces.Discrete(4)
    policies = {
        "supervisor_policy": (None,obs_supervisor,acti, {}),# <- use default class & infer obs-/act-spaces from env.
        "operator_policy": (None,obs_operator,acti, {}),# <- use default class & infer obs-/act-spaces from env.
    }
